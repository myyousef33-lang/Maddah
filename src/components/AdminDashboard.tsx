...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate }) => {
  ...
  const [bonusReasonTitle, setBonusReasonTitle] = useState<string>('مكافأة التفوق الرياضي الإضافية');
  const [showBonusModal, setShowBonusModal] = useState(false);

  // AI Assistant Admin State
  const [aiSystemInstruction, setAiSystemInstruction] = useState<string>(
    'أنت معلم وخبير مادة الرياضيات (مداح الرياضيات) لكافة المراحل الدراسية. أجب بدقة علمية وتبسيط مميز، ووضح خطوات الحل الرياضية والقوانين والنظريات بدقة.'
  );
  ...
  const handleExamDateUpdate = (val: string) => {
    ...
    try {
      ...
      if (!isNaN(d.getTime())) {
        ...
        setDateUpdateFeedback('تم حفظ وتطبيق موعد امتحان الرياضيات الجديد والعد التنازلي بنجاح!');
...
لأي استفسار التواصل مع الإدارة: ${settings.whatsappNumber}`;
  ...
  const handleOpenSupportWhatsapp = () => {
    ...
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(`السلام عليكم، استفسار بخصوص الدعم الفني للوحة إدارة منصة ${settings.platformName || 'مداح الرياضيات'}`)}`, '_blank');
  };
          ...
          <div className="flex items-center justify-between gap-3 sm:gap-6">
            ...
            <div className="hidden lg:flex items-center gap-3 rounded-2xl border border-slate-200 bg-[#F5F7FA] px-4 py-2 shadow-xs">
              ...
              <div className="text-right">
                ...
                <p className="text-[10px] text-[#6B7280] mt-0.5">منصة {settings.platformName || 'مداح الرياضيات'} التعليمية</p>
              </div>
            </div>
          ...
          <aside className="hidden lg:block w-72 shrink-0 lg:sticky lg:top-24 space-y-4">
            ...
            <div className="rounded-3xl border border-slate-200 bg-white p-4 shadow-xs">
              <div className="flex items-center gap-3 mb-3">
                <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#06090F] border border-[#D4AF37]/40 text-[#F3C644] font-black text-xl shadow-xs">
                  ∑
                </div>
                <div>
                  <h3 className="font-black text-[#0D1B3E] text-base leading-tight">{settings.platformName || 'مداح الرياضيات'}</h3>
              ...
              <div className="space-y-6">
                ...
                {overviewSectionOrder.map((sectionKey, index) => {
                  ...
                  const sectionLabels: Record<string, string> = {
                    ...
                    countdown_control: '4. التحكم في موعد امتحان الرياضيات الوزاري'
                  };
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#6B7280] shadow-xs">
                  <div className="flex items-center gap-2.5">
                    <Logo size="sm" showSubtitle={false} />
                    <span>{settings.platformName || 'مداح الرياضيات'} - منصة التفوق والتميز في الرياضيات • جميع الحقوق محفوظة © {new Date().getFullYear()}</span>
                  </div>
                            ...
                            if (file) {
                              handleFileUpload(file, 'pdf', (dataUrl, fileName, fileSize) => {
                                setPdfForm({ 
                                  ...pdfForm, 
                                  url: dataUrl,
                                  title: pdfForm.title || fileName?.replace(/\.[^/.]+$/, '') || 'مذكرة رياضيات',
          ...
          {showAddChallenge && (
            <div className="rounded-3xl border border-slate-200 bg-white p-6 space-y-4 shadow-sm animate-in fade-in">
              ...
              <form onSubmit={(e) => {
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-[#0D1B3E]">نص السؤال أو المسألة الرياضية</label>
                  <textarea
                    rows={3}
                    value={challengeForm.qText}
                    onChange={e => setChallengeForm({ ...challengeForm, qText: e.target.value })}
                    placeholder="أدخل نص السؤال أو المسألة الرياضية بالتفصيل..."
                    className="w-full rounded-xl border border-slate-200 bg-[#F5F7FA] p-2.5 text-xs text-[#0D1B3E]"
                    required
                  />
                </div>
                            ...
                            onClick={() => {
                              ...
                              if (!enrolled.includes(quickCourseSelect)) {
                                ...
                                StorageService.sendNotification({
                                  ...
                                  message: `تم تفعيل كورس "${course?.title || 'كورس رياضيات'}" لحسابك بقرار مباشر من إدارة المنصة.`,
                                  target: 'student',
                                  targetStudentId: updated.id
                                });
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="مثال: بطل الرياضيات في امتحان التفاضل والتكامل"
                  className="w-full rounded-xl border border-slate-200 bg-[#F5F7FA] p-2.5 text-[#0D1B3E]"
                />
              </div>
            </div>
