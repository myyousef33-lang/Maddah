...
export const CertificateModal: React.FC<CertificateModalProps> = ({
  ...
  const drawCertificate = () => {
    ...
    ctx.fillText('منصة مداح الرياضيات التعليمية - Maddah Mathematics LMS', 600, 125);

    ctx.fillStyle = '#6B7280';
    ctx.font = '18px sans-serif';
    ctx.fillText('تحت إشراف وتوجيه خبير مادة الرياضيات الأستاذ / مداح', 600, 160);
    ...
    ctx.textAlign = 'left';
    ctx.fillText('اعتماد: مداح الرياضيات', 150, 735);
  };
  ...
  const handleDownload = () => {
    ...
    link.download = `Maddah_Math_Certificate_${student.name.replace(/\s+/g, '_')}.png`;
    link.href = dataUrl;
    link.click();
  };

  const handleShare = () => {
    const text = `حصلت على شهادة تفوق في الرياضيات من منصة مداح الرياضيات بنسبة ${certificate.percentage}% في ${certificate.examOrUnitName}!`;
    const shareUrl = window.location.origin;

    if (navigator.share) {
      navigator.share({
        title: 'شهادة تفوق مداح الرياضيات',
        text: text,
        url: shareUrl
      }).catch(() => {});
    } else {
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-4 mb-4">
          <div className="flex items-center gap-2">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-50 border border-amber-200 text-[#F97316]">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-5 border-t border-slate-100 mt-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Share2 className="h-4 w-4 text-[#FDBA74]" />
              <span>مشاركة الرابط</span>
            </button>

            <button
              onClick={handleDownload}
              className="rounded-xl bg-[#F97316] px-5 py-2.5 text-xs font-black text-[#0D1B3E] hover:bg-[#F97316] shadow-xs transition-all flex items-center gap-2"
            >
