...
const STORAGE_KEYS = {
  ...
  CURRENT_STUDENT: 'maddah_math_db_current_student_v1',
  COURSES: 'maddah_math_db_courses_v1',
  EXAMS: 'maddah_math_db_exams_v1',
  ATTEMPTS: 'maddah_math_db_exam_attempts_v1',
  PROGRESS: 'maddah_math_db_lesson_progress_v1',
  LAST_VIEWED: 'maddah_math_db_last_viewed_lessons_v1',
  KEYS: 'maddah_math_db_activation_keys_v1',
  PDF_CATEGORIES: 'maddah_math_db_pdf_categories_v1',
  PDF_FILES: 'maddah_math_db_pdf_files_v1',
  NOTIFICATIONS: 'maddah_math_db_notifications_v1',
  SETTINGS: 'maddah_math_db_settings_v1',
  ADMIN_AUTH: 'maddah_math_db_admin_auth_v1',
  ADMIN_TOKEN: 'maddah_math_admin_jwt_token_v1',
  WEAKNESS_PROFILES: 'maddah_math_db_weakness_v1',
  LEADERBOARD: 'maddah_math_db_leaderboard_v1',
  WEEKLY_CHALLENGES: 'maddah_math_db_weekly_challenges_v1',
  AI_CHAT_HISTORY: 'maddah_math_db_ai_chat_history_v1',
  PAYMENT_METHODS: 'maddah_math_db_payment_methods_v1',
  WALLET_TRANSACTIONS: 'maddah_math_db_wallet_transactions_v1',
  AUDIT_LOGS: 'maddah_math_db_audit_logs_v1',
  LESSON_COMMENTS: 'maddah_math_db_lesson_comments_v1',
  STUDY_RECOMMENDATIONS: 'maddah_math_db_study_recommendations_v1',
  ASSIGNMENTS: 'maddah_math_db_assignments_v1',
  ASSIGNMENT_SUBMISSIONS: 'maddah_math_db_assignment_submissions_v1'
};
...
export const hashPassword = async (password: string): Promise<string> => {
  try {
    ...
    const salt = 'maddah_math_sec_salt_2026_';
    ...
    return 'hashed_' + btoa(password + '_salt_maddah');
  }
};
...
export const verifyPassword = async (inputPassword: string, storedHashOrPlain?: string): Promise<boolean> => {
  ...
  if (storedHashOrPlain.startsWith('hashed_')) {
    return 'hashed_' + btoa(cleanInput + '_salt_maddah') === storedHashOrPlain;
  }
...
try {
  if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
    broadcastChannel = new BroadcastChannel('maddah_math_storage_sync');
...
const SEED_SETTINGS: PlatformSettings = {
  platformName: 'مداح الرياضيات | منصة الرياضيات لجميع المراحل',
...
export const StorageService = {
  ...
  async registerStudent(data: {
    ...
    // Auto-enroll new student into their grade's primary course
    const allCourses = this.getCourses();
    const matchingCourses = allCourses.filter(c => c.grade === data.grade);
    const initialEnrolledIds = matchingCourses.length > 0
      ? matchingCourses.map(c => c.id)
      : (allCourses.length > 0 ? [allCourses[0].id] : []);
  ...
  createCourse(data: Partial<Course>): Course {
    const newCourse: Course = {
      id: 'crs-' + Date.now(),
      title: data.title || 'كورس رياضيات جديد',
  ...
  isStudentEnrolled(studentId: string, courseId: string): boolean {
    ...
    if (!student) {
      ...
      if (cur && cur.id === studentId) student = cur;
    }
  ...
  activateKey(studentId: string, rawCode: string): { success: boolean; message: string; type?: 'course' | 'pdf'; itemTitle?: string } {
    ...
    if (keyType === 'course') {
      ...
      return {
        success: true,
        message: `تهانينا! تم تفعيل الاشتراك بنجاح في: ${key.targetTitle || key.targetName || 'كورس الرياضيات'}!`,
        type: 'course',
        itemTitle: key.targetTitle || key.targetName || 'كورس الرياضيات'
      };
    } else if (keyType === 'pdf') {
  ...
  createPdf(data: Partial<PdfFile>): PdfFile {
    const newPdf: PdfFile = {
      ...
      title: data.title || 'مذكرة رياضيات جديدة',
      description: data.description || '',
  ...
  recordExamWeaknesses(attempt: ExamAttempt, exam: QuizExam): StudentWeaknessProfile {
    ...
    exam.questions.forEach((q, idx) => {
      ...
      const topicName = q.explanation?.split('.')[0]?.trim() || (exam as any).unitTitle || exam.title || 'مسائل الرياضيات';
    ...
    const defaultMethods: PaymentMethod[] = [
      {
        ...
        accountName: 'مداح الرياضيات (حساب المحفظة الرسمي)',
      ...
      },
      {
        ...
        type: 'instapay',
        accountNumber: 'maddah_math@instapay',
        accountName: 'منصة مداح الرياضيات التعليمية',
      ...
      },
      {
        ...
        accountName: 'كود خدمة منصة مداح الرياضيات',
