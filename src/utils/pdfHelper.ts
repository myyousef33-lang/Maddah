...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 1130" width="100%" height="100%">
  <defs>
    <style>
      ...
      .gold-accent { fill: #F97316; }
      .card { fill: #FFFFFF; stroke: #E2E8F0; stroke-width: 2; rx: 16; }
      .title { font-size: 24px; font-weight: 900; fill: #FFFFFF; text-anchor: middle; }
      .subtitle { font-size: 14px; font-weight: 700; fill: #F97316; text-anchor: middle; }
      .q-num { font-size: 16px; font-weight: 900; fill: #F97316; }
  ...
  <g transform="translate(40, 200)">
    ...
    <text x="700" y="35" class="q-num" text-anchor="end">السؤال الأول (4 درجات) - التفاضل والتكامل:</text>
    <text x="700" y="65" class="q-text" text-anchor="end">إذا كانت د(س) = س³ - 3س + 5، أوجد فترات التزايد والتناقص للدالة</text>
    <text x="700" y="90" class="q-text" text-anchor="end">ثم حدد النقط الحرجة وقيم النهايات العظمى والصغرى المحلية موضحاً خطوات الحل الرياضية.</text>
    ...
    <text x="670" y="220" font-size="13" fill="#94A3B8" text-anchor="end">مساحة كتابة الخطوات وجدول الإشارات والناتج النهائي هنا...</text>
  </g>

  <!-- Question 2 -->
  <g transform="translate(40, 480)">
    <rect x="0" y="0" width="720" height="260" class="card" />
    <text x="700" y="35" class="q-num" text-anchor="end">السؤال الثاني (4 درجات) - الهندسة الفراغية:</text>
    <text x="700" y="65" class="q-text" text-anchor="end">أوجد معادلة الخط المستقيم المار بالنقطة أ(2، -1، 3) والمتجه هـ = (1، 4، -2) متجهاً لاتجاهه،</text>
    <text x="700" y="90" class="q-text" text-anchor="end">ثم اكتب الصورة الإحداثية والمتجهة والبارامترية للخط المستقيم في الفراغ.</text>

    <!-- Formula Box -->
    <rect x="30" y="105" width="660" height="70" fill="#F8FAFC" rx="10" stroke="#E2E8F0" />
    <text x="670" y="132" class="formula" text-anchor="end">r = r₀ + t · d  ⇒  (x, y, z) = (2, -1, 3) + t(1, 4, -2)</text>
    <text x="670" y="158" class="formula" text-anchor="end">(x - 2)/1 = (y + 1)/4 = (z - 3)/-2 = t</text>

    <!-- Answer Box -->
    <rect x="30" y="185" width="660" height="60" class="answer-box" />
    <text x="670" y="220" font-size="13" fill="#94A3B8" text-anchor="end">اكتب الصور المختلفة لمعادلة المستقيم في الفراغ هنا...</text>
  </g>

  <!-- Question 3 -->
  <g transform="translate(40, 760)">
    <rect x="0" y="0" width="720" height="240" class="card" />
    <text x="700" y="35" class="q-num" text-anchor="end">السؤال الثالث (4 درجات) - حساب المثلثات والتكامل:</text>
    <text x="700" y="65" class="q-text" text-anchor="end">احسب قيمة التكامل المحدد: ∫ من 0 إلى π/2 [ جا(س) · جتا(س) ] د س</text>
    <text x="700" y="90" class="q-text" text-anchor="end">باستخدام المتطابقة المثلثية لضعف الزاوية جا(2س) = 2 جا(س) جتا(س).</text>

    <rect x="30" y="110" width="660" height="110" class="answer-box" />
    <text x="670" y="145" font-size="13" fill="#94A3B8" text-anchor="end">اكتب خطوات التعويض وحساب قيمة التكامل المحدد هنا...</text>
  </g>

  <!-- Footer -->
  <text x="400" y="1080" font-size="13" font-weight="700" fill="#64748B" text-anchor="middle">منصة مداح الرياضيات التعليمية • بالتوفيق والتميز الدائم لجميع الأبطال</text>
</svg>
`.trim();
...
export const generateSampleMathWorksheetDataUrl = generateSamplePhysicsWorksheetDataUrl;
 ...
 */
export async function downloadPdfFile(rawUrl: string, fileName = 'مذكرة_رياضيات.pdf'): Promise<void> {
  if (!rawUrl) return;
